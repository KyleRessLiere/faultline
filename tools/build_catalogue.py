...
def render(fights, verdicts):
    ...
    w("ground, `X` a breakable blocker, `*` a deployment spot, and any other letter an enemy from")
    w("that battle's legend. A unit never starts on a hazard — the tile under a spot or a spawn is")
    w("Open.\n")
    w("A spot belongs to **neither player** (MASTER_DESIGN §3's deployment draft): either flock may")
    w("draft into any open one, and spot layout is an authoring axis in its own right — the same")
    w("terrain drafted from clustered spots and from scattered spots is two different fights. Boards")
    w("still showing `A`/`B` have not been migrated yet; their two zones are read as one shared list.\n")
