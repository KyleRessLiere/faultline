...
9×7 · objective **kill all** · 4× Runt, 1× Colossus, 1× Lobber (34 HP of fighters) · 8 deployment spots · 1 reinforcement wave(s)
...
- THE LOBBER IS THE ANTI-TURTLE CLAUSE. It sits at 4,1 above the gap in the northern masonry, Move 2 and range 3, and it retreats the moment anything closes on it. Standing still and waiting for the Colossus to arrive costs 2 a round for as long as you do it, which is the price this board charges for the patient answer rather than forbidding it.
- SPOT LAYOUT (MASTER_DESIGN 3, the deployment draft). Eight spots, two 2x2 pockets, all unowned, and every one of them outside every enemy's round-1 reach - the strict form of agency before injury (D-080). A Runt is Move 4 and reach 1, which is a diamond of 5 on open ground; the masonry at 1,1 2,1 1,2 and 6,1 7,1 7,2 is what keeps it from getting there, because reach is measured through the walk and the walk has to go round. The Lobber's deepest stand is 4,3 and its range is 3, which stops two tiles short of row 5.
...
> Raiders walk two channels at a twelve-hit-point pumphouse and never once look at you. The east channel ends in masonry and the west channel ends in a drain, and a Bulwark decides which of the two you can still afford.
...
- THE ROUND-3 QUESTION - the second pair of Raiders lands at the start of round 3, one at each end of the board, and by then the Bulwark has caught up with the first pair. The question is which wall you fight against for the rest of the fight: the east masonry, where a one-tile shove is 4 and the aura changes nothing at all, or the west drain, where the same removal needs two tiles of travel and therefore costs you the Bulwark first.
- THE TWO CHANNELS ARE NOT THE SAME ROUTE DRAWN TWICE, AND THAT IS THE BOARD. East is a solid three-tile bank of masonry at 5,1 5,2 5,3. West is the same bank with its middle tile replaced by a drain: 1,1 wall, 1,2 DRAIN, 1,3 wall. A collision into masonry is 4 and a Stagger, which kills a 4-hit-point Raider outright and does nothing whatever to a 10-hit-point Bulwark. A shove into the drain is Clinging and then permanent, and it does not care what the hit points were. Same verb, same one action, two completely different prices - and the expensive answer is the only one that removes the aura.
- WHERE THE AURA ACTUALLY BITES, IN TILES. HoldAura caps the displacement of every ally ADJACENT to the Bulwark at one tile - never its own carrier (D-019), never to zero (MASTER_DESIGN 2, gradients not immunities). A Raider standing on 2,2 needs one tile of shove to reach the drain at 1,2, and one tile is exactly what the cap allows, so the aura is irrelevant there. A Raider standing on 3,2 needs two, and inside the aura it travels one and stops on 2,2 with the drain still a tile away. The aura is not a wall across the drain; it is a tile of extra walking, paid before every shove, until somebody spends ten hit points on it.
- HOLD CAPS DISTANCE, NOT DAMAGE, and the east bank is the proof. A Raider adjacent to the masonry is one tile from a collision whatever else is standing beside it, so the aura never protects anybody from the east wall. That is why the cheap answer stays available all game and why this board is a price gap rather than a lock.
- THE RAIDERS DO NOT CARE ABOUT YOU. PlanRaider has exactly two clauses - claw the structure if adjacent, else path to it - and no clause about player units at all, so nothing you do to them personally makes them stop wanting the pumphouse. A Raider is 4 hit points, Move 3 and 2 damage a claw, and its intent names the structure and prints the hit points it will have left (D-164). Twelve hit points and two Raiders adjacent is three rounds. The escort Husk DOES hunt you, so standing on the ring and swinging is not a plan.
- SHOVING AN ENEMY INTO THE THING YOU ARE GUARDING DAMAGES THE THING YOU ARE GUARDING. A collision into a structure is source-blind and lands the full 6, which is half the pumphouse for one careless push. The ring tiles 2,3 4,3 3,2 and 3,4 are all places where an aimed shove has a wrong answer, and the board never says which direction is which - the preview does.
- TWO ROUTES, UNEQUAL PRICES, AND THE SPOT POCKETS ARE DELIBERATELY LOPSIDED. Four spots sit east at 4,6 5,6 6,6 and 6,5; two sit west at 0,6 and 1,6. From 5,6 the east bank is three steps - 5,5 to 5,4 to 4,4 - and a duck there is answering the east channel from round 2 at 4 damage a slam. From 0,6 the west drain's firing tile at 0,2 is four steps and its useful tile 2,3 is five, and from either of them the east channel is five tiles away and cannot be answered at all. Cheap, near and shallow against dear, far and decisive.
- NO TURN LIMIT, AND THAT IS A FORMAT FACT RATHER THAN A CHOICE. The format refuses a deadline on protect - it cannot be won by the bell (D-167) - so the win here is clearing both channels and the loss is the pumphouse falling. Recorded rather than worked around, exactly as the-shrine records it.
- TERRAIN BUDGET. Six impassable tiles in two connected formations of three, plus the structure: 14.3 percent with the structure counted, below the 15 percent floor and legal because a non-kill-all objective supplies the pressure directly - G3's third clause, the amendment the Warrens cross-reading made and the reason all five of its non-kill-all boards are sound below the floor. ONE drain on the whole board against four wall tiles, because a pit is the finisher and should feel rare.
- SPOT LAYOUT (MASTER_DESIGN 3, the deployment draft). Six spots, unowned, and the lopsided pockets are the thesis stated at deployment: both flocks may pile into the eastern cluster and concede the west channel, or split and cover both at half strength. Every spot is outside every enemy's round-1 reach - a Raider's walk is 3 and its swing 1, and the banks at x1 and x5 are what force the walk round rather than down.
...
- SPEND ACTIONS OR SPEND THE ENEMY. Any attack chips masonry for 2 whatever the weapon (D-060), so sixteen hit points is eight direct actions - the costly baseline that always exists and always works. A collision does full damage at 6, so three clean slams is the fast route, and the bodies to slam are the ones walking at you. The Court supplies its own ammunition, and the gate does not care who pushed.
...
- TWO ROUTES, UNEQUAL PRICES, in numbers. THE THROAT - from the spot at 8,2 walk 8,3 - 7,3 - 6,3 - 5,3. Four steps to the only tile that slams him, and the corridor between the shelves is one tile wide so exactly one duck is ever in contact. It costs six damage a round from a melee that also pushes you back a tile, so you pay one AP every round just to stay in it, and nothing you do from that tile ever puts him in a drain. THE NORTH FINGER - from the spot at 8,0 walk 7,0 - 6,0 - 6,1 - 7,1 is the wrong way; the real line is 7,0 - 6,0 - 5,0 - 4,0 - 4,1 - 4,2, six steps and two of them priced: 4,0 is spikes and costs 2 hit points to cross (SpikeWalkDamage), and 4,1 is open ground the Husks and the west Lobber both cover. Two AP dearer and paid for in blood - and it is the route that reaches the drain at 5,1, the rim the whole endgame is about.
