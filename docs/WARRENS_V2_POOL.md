...
| **Ordinary** | 31 | The bulk of an act — its early and middle columns, and the band Warrens v2 draws most heavily from. |
| **Hard** | 19 | The late third: the fights a squad arrives at already spent. |
...
**65 boards in total**, and 38 retired ones the generator never sees.
...
- SPEND ACTIONS OR SPEND THE ENEMY, and both routes are real. Any attack chips masonry for a flat 2 whatever the weapon (D-060, D-281) - an Archer's sweet spot chips for 2 exactly as a Vanguard's swing does, because the number is about the masonry and not about the arm - so twelve hit points is six direct actions, the patient answer any roster can pay. A collision does full damage at 6, so TWO CLEAN SLAMS is the fast one, and the bodies to slam are the ones walking at you. Four gate faces, two prices, and the Court supplies the ammunition for the cheaper one.
- THE ARITHMETIC WAS COUNTED TWICE AND WRONG BOTH TIMES BEFORE IT WAS COUNTED RIGHT, and that is recorded rather than tidied away. The first cut sat at 16 hit points behind a sealed wall bar, which left the gate ONE reachable face at 4,2 and a squad chipping at a clock it could not beat. The second correction went the other way: it declared the swing route unavailable to everyone but a Wardbearer, which was true of the CODE at the time and false of the RULE - D-060 has always said any attack chips for 2, and the player's basic attack simply could not be aimed at a structure. D-281 made the rule reachable. The lesson worth keeping is that a design note asserting arithmetic is a claim about the build, not about the doc, and it has to be measured against the build.
