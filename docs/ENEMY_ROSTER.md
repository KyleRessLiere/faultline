...
|---|---|---|---|---|
| **Husk** | 4 | 3 | melee 2 | Adjacent → attack, else close. **Shoulder**: barrels through a body on its route, knocking it 1 aside for 2 and paying +1 MP (D-100). Chaff. Dies to one collision. |
| **Lobber** | 6 | 2 | range 3, 2 | Holds a 2–3 band and lobs; retreats if you close. Bodies do not block it. |
| **Anchor** | 12 | 1 | melee 4 | Shrugs one tile off every Push. Adjacent → attack, else advance. |
| **Grappler** | 10 | 3 | pull 2, range 3 | Pulls toward itself, preferring HighGround targets then the Archer. Inert in melee. |
| **Stalker** | 8 | 4 | push 1, range 1 | Shoves players toward hazards, ranking pit > spikes > wall/edge. |
...
4. **Two "no damage" units are the best damage in the game.** The Stalker's edge shove is a
   guaranteed 4 + Stagger per round; a Grappler pulling a target into an ally is 8 per round.
...
`HP 12 · Move 0 · melee 4 · 2 negating Footing tokens`
...
`HP 6 · Move 2 · range 3, 2 dmg`
A Lobber that scores elevation: it seeks the nearest HighGround it can reach, holds it, and fires at
+2 from there. Fills gap 2, and makes the Archer's favourite tile a contested objective rather than
free real estate.
...
`HP 10 · Move 2 · melee 2 · passive: adjacent allies cannot be displaced more than 1`
...
`HP 20 · Move 1 · melee 6 · Push resistance 2`
...
| **Heavy Husk** | Husk | HP 6 | Survives one collision. Tests how much of the Husk's role is "dies to a shove". |
