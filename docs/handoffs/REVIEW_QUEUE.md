...
| 0b | **A kill-all wins a Destroy board, which §7 says it cannot.** `Objectives.Check` returns `Win` on `!AnyEnemyLeft` under **every** objective (D-032). `break-the-gate` declares `destroy 3,1 hp 18` correctly, so this is a **win-condition resolution defect, not a policy-scoring one** — the policies clear the board because clearing it genuinely wins, and they are exonerated. Affects every Destroy board ever authored. Also: that board declares **no `turn-limit:`**, so §7's "turn-limit expiry is a loss" has nothing to expire. | The Destroy objective is untested by the harness and unenforced in play. Stage F's gate cannot be judged until it is. | D-167, D-186 correction |
...
| 6b | **Stage D item 6 cannot certify while `--seed` is inert.** The sweep asserts *card-level distribution across seeds*; with nothing in Core consuming an RNG inside a fight, that assertion is **unfalsifiable — it passes on a director that returns a constant**. Held, and gated on Stage F's F3 (the seeded generator) and F4 landing together. | Item 6, and with it Stage D's completion. It also reopens the *evidence* behind the Camp 1 ruling: "Technique, Technique for every seed 1–40" equally supports "the director's weighting" and "no seed reached the director". The **ruling** survives as authored intent; the **finding** that prompted it was never established, and the Engine Starter distribution question is untestable until the seed is live. | §14 #13 |
