...
| 0b | **No evaluator policy ever attacks a structure** — zero collisions, zero chips, zero destroyed, on every board and every policy, at collision 4 and at 6 alike. | `break-the-gate`'s Destroy objective is **untested by the harness**, and its runs are decided by the kill-all win §7 forbids. Stage F's gate cannot be judged until the harness can play a Destroy board. | D-186 correction |
