...
**72 active boards** (27 retired). Nothing marks difficulty: a `.fight` carries `id · name · number · size ·
...
**Whole act:** 30 combat nodes at the worst seed against 62 boards in the three bands the generator draws from — enough, if every band may fill any column.

### What that means in one paragraph

The **floor is met with room to spare** — 72 active boards
...
it and together ask for up to 21 against 33 boards. Hard is comfortable at 22 for 8.
...
4. **A second elite board.** Not a shortfall for one act — v2 places exactly one — but one
   ...
   hungry route is *for*.
5. **Hard is comfortable** at 22 for 8, and **Endurance (4) is unreachable** —
