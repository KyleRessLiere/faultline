...
namespace Faultline.Core
{
        ...
        IReadOnlyList<Coord> Tiles)
    {
            ...
            switch (objective.Kind)
            {
                ...
                case ObjectiveKind.Protect:
                {
                    ...
                    return new ObjectiveStatus(
                        ...
                        hp,
                        max,
                        "Structure " + Number(hp) + "/" + Number(max),
                        clock,
                ...
                case ObjectiveKind.Destroy:
                {
                    ...
                    return new ObjectiveStatus(
                        objective.Kind,
                        "Bring the structure down. Collisions only — attacks chip it for 1.",
                        state.Fight.TurnLimit > 0
                            ? "Run out of turns, or lose every unit."
                            : "Lose every unit.",
                        max - hp,
                        max,
                        "Structure " + Number(max - hp) + "/" + Number(max) + " down",
                        clock,
                        clockTight,
                        tiles);
                }
        ...
        private static readonly ObjectiveStatus Empty = new ObjectiveStatus(
            ...
            string.Empty, string.Empty, false, new Coord[0]);

        private static (int Hp, int Max) StructureHp(GameState state)
        {
            int hp = 0;
            int max = 0;
